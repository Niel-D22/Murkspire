...
type ColorBendsProps = {
  ...
  speed?: number;
  colors?: string[];
  /** Single-colour shorthand (reactbits.dev API). Takes precedence over colors. */
  color?: string;
  /** 0-1: how far down the canvas fades in from transparent at the top. */
  fadeTop?: number;
  transparent?: boolean;
  autoRotate?: number;
  scale?: number;
...
export default function ColorBends({
  ...
  const paletteKey = (color ? [color] : colors || []).filter(Boolean).join(',');
  const palette = useMemo(() => (paletteKey ? paletteKey.split(',') : []), [paletteKey]);
  ...
  const fade =
    fadeTop > 0
      ? `linear-gradient(to bottom, transparent 0%, black ${Math.min(fadeTop, 1) * 100}%)`
      : undefined;
  ...
  return (
    <div
      ...
      style={{ ...style, WebkitMaskImage: fade, maskImage: fade }}
    />
  );
}
